...
# CS2 更新后的骨骼索引（头部从6改为7）
SKELETON_BONES = {
    # 躯干核心链
    7: [6],  # 头 -> 脖子 (原头部是6，现在是7)
    6: [5],  # 脖子 -> 胸口 (原脖子是5，现在是6)
    5: [4],  # 胸口 -> 胃 (保持不变)
    4: [3, 13, 8],  # 胃 -> 中间点, 右肩膀，左肩膀 (保持不变)
    3: [2],  # 中间点 -> 小头 (保持不变)
    2: [1],  # 小头 -> 骨盆 (保持不变)
    1: [0],  # 骨盆 -> 根部 (保持不变)
            ...
            try:
                ...
                with np.errstate(divide='ignore', invalid='ignore'):
                    ...
                    if np.all(valid_indices):
                        ...
                        # 验证坐标是否在屏幕范围内
                        elif not Entity.validate_screen_position(pos2d) and not Entity.validate_screen_position(head_pos2d):
