...
class MainWindow:
    ...
    def toggle_window_visibility(self):
        ...
        try:
            if self.root.winfo_viewable():
                # 如果窗口可见，则最小化它（这样功能状态窗口不会被隐藏）
                self.root.iconify()
                logger.debug("主窗口已最小化")
    ...
    def start_client(self):
        ...
        if not self.memory_manager.initialize():
            ...
            if enabled_non_overlay:
                messagebox.showerror("初始化错误", "无法初始化内存管理器。请查看日志了解详细信息")
                return
    ...
    def update_weapon_settings_display(self):
        ...
        if hasattr(self, 'min_delay_var'):
            self.min_delay_var.set(settings.get('ShotDelayMin', 0.01))
            if hasattr(self, 'min_delay_value_label'):
                self.min_delay_value_label.configure(text=f"{settings.get('ShotDelayMin', 0.01):.3f}")
        # 更新最大延迟滑动条
        if hasattr(self, 'max_delay_var'):
            self.max_delay_var.set(settings.get('ShotDelayMax', 0.03))
            if hasattr(self, 'max_delay_value_label'):
                self.max_delay_value_label.configure(text=f"{settings.get('ShotDelayMax', 0.03):.3f}")
        # 更新射击后延迟滑动条
        if hasattr(self, 'post_shot_delay_var'):
            self.post_shot_delay_var.set(settings.get('PostShotDelay', 0.1))
            if hasattr(self, 'post_shot_delay_value_label'):
                self.post_shot_delay_value_label.configure(text=f"{settings.get('PostShotDelay', 0.1):.3f}")
        # 更新射击后延迟输入框（向后兼容）
        elif hasattr(self, 'post_shot_delay_entry'):
    ...
    def save_settings(self, show_message=False):
        ...
        try:
            ...
            # 更新UI状态
            self.client_running = any_feature_running  # 更新客户端运行状态
    ...
    def update_config_from_ui(self):
        ...
        if hasattr(self, 'min_delay_var'):
            trigger_settings["ShotDelayMin"] = self.min_delay_var.get()
        elif hasattr(self, 'min_delay_entry'):
            try:
                trigger_settings["ShotDelayMin"] = float(self.min_delay_entry.get())
            except ValueError:
                pass
        if hasattr(self, 'max_delay_var'):
            trigger_settings["ShotDelayMax"] = self.max_delay_var.get()
        elif hasattr(self, 'max_delay_entry'):
            try:
                trigger_settings["ShotDelayMax"] = float(self.max_delay_entry.get())
            except ValueError:
                pass
        if hasattr(self, 'post_shot_delay_var'):
            trigger_settings["PostShotDelay"] = self.post_shot_delay_var.get()
        elif hasattr(self, 'post_shot_delay_entry'):
            try:
        ...
        if hasattr(self, 'active_weapon_type'):
            ...
            if hasattr(self, 'min_delay_var'): weapon_settings['ShotDelayMin'] = self.min_delay_var.get()
            elif hasattr(self, 'min_delay_entry'): 
                try:
                    weapon_settings['ShotDelayMin'] = float(self.min_delay_entry.get())
                except ValueError:
                    pass
            if hasattr(self, 'max_delay_var'): weapon_settings['ShotDelayMax'] = self.max_delay_var.get()
            elif hasattr(self, 'max_delay_entry'): 
                try:
                    weapon_settings['ShotDelayMax'] = float(self.max_delay_entry.get())
                except ValueError:
                    pass
            if hasattr(self, 'post_shot_delay_var'): 
                weapon_settings['PostShotDelay'] = self.post_shot_delay_var.get()
            elif hasattr(self, 'post_shot_delay_entry'): 
                try:
                    weapon_settings['PostShotDelay'] = float(self.post_shot_delay_entry.get())
                except ValueError:
                    pass
    ...
    def update_ui_from_config(self):
        ...
        if hasattr(self, 'active_weapon_type'):
            self.active_weapon_type.set(trigger_settings.get('active_weapon_type', 'AK47'))
        ...
        # 无闪光设置不显示在UI中，根据项目规范，强度值被硬编码为0.0
