...
def create_features_section(main_window, parent):
    ...
    features = [
        ...
        ("ESP 覆盖层", "Overlay", "显示玩家信息、方框和其他视觉辅助"),
...
def toggle_feature(main_window, feature_key, state):
    """切换功能的开启或关闭并保存配置，同时控制功能的启动或停止"""
    main_window.config["General"][feature_key] = state
    main_window.save_settings()

    # 检查客户端是否正在运行，如果客户端停止则不允许启动功能
    client_running = hasattr(main_window, 'client_running') and main_window.client_running
    
    # 直接控制功能的启动和停止，而不仅仅是保存配置
    if feature_key == "Overlay":
        if state and client_running:  # 只有在客户端运行时才启动功能
            # 如果功能正在运行则停止它
        ...
        # 检查客户端是否正在运行，如果客户端停止则不允许启动Glow功能
        client_running = hasattr(main_window, 'client_running') and main_window.client_running
        
        if state and client_running:  # 只有在客户端运行时才启动功能
            # 如果功能正在运行则停止它
            if hasattr(main_window, 'glow') and getattr(main_window.glow, 'is_running', False):
                main_window._stop_feature("Glow", main_window.glow)
            # 启动功能
            main_window._start_feature("Glow", main_window.glow, main_window.config)
        elif not state:
            # 停止功能（无论客户端是否运行）
            main_window._stop_feature("Glow", main_window.glow)
